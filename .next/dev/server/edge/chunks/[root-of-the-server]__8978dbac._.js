...
function middleware(request) {
    ...
    const sessionToken = request.cookies.get("next-auth.session-token")?.value || request.cookies.get("__Secure-next-auth.session-token")?.value || request.cookies.get("authjs.session-token")?.value || request.cookies.get("__Secure-authjs.session-token")?.value;
...
const config = {
    matcher: [
        "/((?!api|_next/static|_next/image|favicon.ico|icons|uploads|manifest.json|logo\\.png).*)"
    ]
};
}),
]);
